...
| `CodeBeam` | big-text + subscription demo | Five-line `BigText` banner (HI / CODE / BEAM / EUROPE / 2026) that inverts the whole 1-bit panel edge to edge every other tick — black-on-white flips to white-on-black and back. Meant to be left running on the badge during the talk. `:half_height` is the only badge-safe size given the 6×8 font's block glyphs. |
