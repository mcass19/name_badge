defmodule NameBadge.Screen.ExRatatui.CodeBeam do
  ...
  Renders `HI` / `CODE` / `BEAM` / `EUROPE` / `2026` as five centred
  `BigText` lines and inverts the whole 1-bit display edge-to-edge on
  every other tick: black letters on white flip to white letters on a
  solid black field, then back. It sits alongside `Goathi` as a second,
  typographic way for ex_ratatui to say hi at the conference.
  ...
  ## Why this size and these five lines

  `BigText` builds large letters out of block glyphs. The badge's 6×8
  font only carries `█ ▀ ▄` (full and half blocks), so only the
  `:full` and `:half_height` pixel sizes render — the narrower sizes
  emit quadrant/half-width glyphs the font lacks. Both safe sizes are
  8 cells wide per character, which caps a line at ~8 characters
  (66 ÷ 8). `CODE BEAM` is nine, so it splits across two lines and the
  banner stacks five lines at `:half_height`.
  ...
  flipping the reverse-video flag on each of its spans. A whole-screen
  invert toggles every pixel each tick, which is the self-clearing case
  for e-ink, so there is no ghosting to manage and the adapter stays on
  partial refresh.
  ...
  | `home`    | A (long press)   | Reset to tick 0 (normal)|
  ...
  @lines ~w(HI CODE BEAM EUROPE 2026)
  @line_height 4
  @line_gap 1
  ...
  def render(state, frame) do
    ...
    fill(frame, inverted?) ++
      banner_lines(frame, inverted?) ++
      [{hint(state, inverted?), hint_rect(frame)}]
  end

  @doc """
  Whether the banner renders inverted (paper letters on an ink field)
  ...
  defp banner_lines(frame, inverted?) do
    style = if inverted?, do: %Style{modifiers: [:reversed]}, else: %Style{}

    @lines
    |> Enum.with_index()
    |> Enum.map(fn {text, index} ->
      widget = BigText.new(text, pixel_size: :half_height, alignment: :center, style: style)
      {widget, line_rect(frame, index)}
    end)
  end

  # Stack the five fixed-height lines, gap between each, vertically
  # centred in the content area above the single-row hint strip.
  defp line_rect(%{width: width, height: height}, index) do
    count = length(@lines)
    block_height = count * @line_height + (count - 1) * @line_gap
